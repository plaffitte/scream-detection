...
##export THEANO_FLAGS=mode=FAST_RUN,device=$device,floatX=float32,OMP_NUM_THREADS=8,openmp='True'
...
# rep_classes="Conversation_Shouting_Noise_BG_voice_Making_Of_Clavier_Compressor_Start_Laugh_Journal_Scream"
Nc=10
...
threshold=10000
max_seq_length=500 #500
n_stream=10
max_batch_length=500 #500

# ################### NETWORK PARAMS ###########################
Nlayers=1                 # number of layers
Ndirs=1                     # unidirectional or bidirectional
Nh=2048                     # hidden layer size
...
STEP1_START=$(date +%s)
if [ $flag -eq 1 ] ; then
  $pythoncmd $curdir/Scripts/format_data_rnn_yun.py --data_type "train" --classes $classes \
                --window_step $window_step --window_size $window_size --highfreq $highfreq --lowfreq $lowfreq \
                --size $size --max_seq_len $max_seq_length --n_stream $n_stream --max_batch_len $max_batch_length \
                 --path $curdir --rep_test $rep_classes --threshold $threshold
  $pythoncmd $curdir/Scripts/format_data_rnn_yun.py --data_type "valid" --classes $classes \
                --window_step $window_step --window_size $window_size --highfreq $highfreq --lowfreq $lowfreq \
                --size $size --max_seq_len $max_seq_length --n_stream $n_stream --max_batch_len $max_batch_length \
                 --path $curdir --rep_test $rep_classes --threshold $threshold
  $pythoncmd $curdir/Scripts/format_data_rnn_yun.py --data_type "test" --classes $classes \
                --window_step $window_step --window_size $window_size --highfreq $highfreq --lowfreq $lowfreq \
                --size $size --max_seq_len $max_seq_length --n_stream $n_stream --max_batch_len $max_batch_length \
                 --path $curdir --rep_test $rep_classes --threshold $threshold
fi
