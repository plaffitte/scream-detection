...
highfreq = 8000 # maximal analysis frequency for mfcc
lowfreq = 50# minimal analysis frequency for mfcc
size = 16 # number of mfcc coef
...
data_vector = np.zeros((1,(nfilt+1)*N),dtype=np.float32)
time = 0
            ...
            try:
                ...
                if label in label_dic:
                    if time < threshold:
                        ...
                        feat,energy = MFSC(signal, samplerate, winstep=window_step, nfft=2048, lowfreq=100,
                                           highfreq=16000, nfilt=nfilt)
                        feat = np.concatenate((feat,energy[:,np.newaxis]),1)
                        L = (stop-start)/10.0**7
                        N_iter = len(feat)/N#math.floor(L/window_step/N)
                        # apply context window
                        if (L/window_step)>N:
                            mfcc_matrix = np.zeros((1,(nfilt+1)*N))
