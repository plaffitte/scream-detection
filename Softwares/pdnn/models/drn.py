...
class DNN(object):
    ...
    def build_finetune_functions(self, train_shared_xy, valid_shared_xy, batch_size):
        ...
        size = T.lscalar('size')  # size of a sequence
        ...
        train_fn = theano.function(inputs=[index, size, theano.Param(learning_rate, default = 0.0001),
              theano.Param(momentum, default = 0.5)],
              outputs=self.errors,
              updates=updates,
              givens={
                self.x: train_set_x[index: (index + size)],
                self.y: train_set_y[index: (index + size)]})

        valid_fn = theano.function(inputs=[index, size],
              outputs=self.errors,
              givens={
                self.x: valid_set_x[index: (index + size)],
                self.y: valid_set_y[index: (index + size)]})
    ...
    def build_extract_feat_function(self, output_layer):
        ...
        feat = T.matrix('feat')
        out_da = theano.function([feat], self.layers[output_layer].output, updates = None,
                                 givens={self.x:feat}, on_unused_input='warn')
        return out_da
