...
coucheRBM="400:1024:1024:18"
coucheDNN="400:1024:1024:1024:18"
layerNumberRBM='2'
layerNumberDNN='3'
epoch_numberRBM=100
epoch_numberDNN=100
lambda="D:0.1:0.8:0.2,0.05"
...
STEP1_START=$(date +%s)
if [ $flag -eq 1 ] ; then
  python Scripts/format_data_mult_env_merge.py --data_type "train" --classes_1 $classes_1 --rep_test $rep_classes \
                --classes_2 $classes_2 --window_step $window_step --window_size $window_size --highfreq $highfreq --size $size \
                --lowfreq $lowfreq --nfilt $size --N $N --slide $slide --exp_path $curdir/.. --threshold $threshold --deltas $compute_deltas
  python Scripts/format_data_mult_env_merge.py --data_type "test" --classes_1 $classes_1 --rep_test $rep_classes \
                --classes_2 $classes_2 --window_step $window_step --window_size $window_size --highfreq $highfreq --size $size \
                --lowfreq $lowfreq --nfilt $size --N $N --slide $slide --exp_path $curdir/.. --threshold $threshold --deltas $compute_deltas
  python Scripts/format_data_mult_env_merge.py --data_type "valid" --classes_1 $classes_1 --rep_test $rep_classes \
                --classes_2 $classes_2 --window_step $window_step --window_size $window_size --highfreq $highfreq --size $size \
                --lowfreq $lowfreq --nfilt $size --N $N --slide $slide --exp_path $curdir/.. --threshold $threshold --deltas $compute_deltas
fi
STEP1_END=$(date +%s)
...
echo $lambda:$epoch_numberCNN
STEP3_START=$(date +%s)
python $pdnndir/cmds/run_CNN.py --train-data "$rep_classes/train.pickle.gz" \
                                --valid-data "$rep_classes/valid.pickle.gz" --nnet-spec $coucheDNN \
                                --conv_nnet_spec $cnn_architecture --wdir $rep_classes/$coucheRBMtxt/$coucheDNNtxt \
                                --l2-reg 0.001 --lrate $lambda:$epoch_numberCNN --model-save-step 10 \
