...
target_path = "/home/piero/Documents/Speech_databases/test/DBN_audio_classification2/data"
        ...
        for j in xrange(len(lines)):
            try:
                ...
                if label in label_dic:
                    ...
                    mfcc = get_mfcc(signal,samplerate,winstep=window_step,nfft=2048,highfreq=4000,lowfreq=100)
                    d1_mfcc = np.zeros((mfcc.shape[0]-1,mfcc.shape[1]))
                    d1_mfcc[0,:] = mfcc[1,:] - mfcc[0,:] 
                    for k in range(1,len(mfcc)-1):
...
#                        merged = np.concatenate((mfcc_matrix,d1_matrix),1)                            
                    ...
                    num_label = label_dic[label]*np.ones(len(mfcc_matrix)-1)#np.ones(len(merged)-1)
                    label_vector = np.append(label_vector,num_label.astype(np.float32,copy=False))
                    data_vector = np.concatenate((data_vector,mfcc_matrix[1:,:].astype(np.float32,copy=False)),0)#np.concatenate((data_vector,merged[1:,:].astype(np.float32,copy=False)),0)
...
shutil.copyfile('/home/piero/Documents/Scripts/format_pickle_data2.py','/home/piero/Documents/Speech_databases/test/DBN_audio_classification2/format_pickle_data.py')
