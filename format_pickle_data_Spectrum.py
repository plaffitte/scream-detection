        ...
        for j in xrange(len(lines)):
            try:
                ...
                if label in label_dic:
                    ...
                    feat,energy = fbank(signal,samplerate,winstep=window_step,nfft=2048,lowfreq=100,highfreq=22050,
                        nfilt=40)
                    feat = np.log(feat)
                    feat = np.concatenate((feat,energy[:,np.newaxis]),1)
                    L = (stop-start)/10.0**7
                    N_iter = len(mfcc)/N#math.floor(L/window_step/N)
                    # apply context window
                    if (L/window_step)>N:
                        feat_matrix = np.zeros((1,13*N))
                        for k in range(int(N_iter)):
                            feat_vec = []
                            for kk in range(N):
                                feat_vec = np.concatenate((feat_vec,feat[k*N+kk,:]))
                            feat_matrix = np.concatenate((feat_matrix,feat_vec[np.newaxis,:]))
                    else:
...
#                if label in label_dic:
                    num_label = label_dic[label]*np.ones(len(feat_matrix)-1)
                    if j==0:
                        data_vector = feat_matrix[1:,:].astype(np.float32,copy=False)
                        ...
                        data_vector = np.concatenate((data_vector,feat_matrix[1:,:].astype(np.float32,copy=False)),0)
                        for k in range(len(label_dic)):
                            if label_dic[label]==k:
                                time_per_occurrence_class[k].append((stop-start)/(10.0**7))
                else:
