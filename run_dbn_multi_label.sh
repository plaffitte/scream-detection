...
STEP1_START=$(date +%s)
if [ $flag -eq 1 ] ; then
    python $curdir/Scripts/format_data_full_multi_label.py --data_type "train" --rep_test $rep_classes --net "DNN"\
                                                                  --param $param_test --classes_1 $classes_1 --classes_2 $classes_2
    python $curdir/Scripts/format_data_full_multi_label.py --data_type "valid" --rep_test $rep_classes --net "DNN"\
                                                                   --param $param_test --classes_1 $classes_1 --classes_2 $classes_2
    python $curdir/Scripts/format_data_full_multi_label.py --data_type "test" --rep_test $rep_classes --net "DNN"\
                                                                   --param $param_test --classes_1 $classes_1 --classes_2 $classes_2
fi
STEP1_END=$(date +%s)
STEP2_START=$(date +%s)
