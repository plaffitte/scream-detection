...
STEP1_START=$(date +%s)
if [ $flag -eq 1 ] ; then
    python $curdir/Scripts/format_data_full_multi_label.py --data_type "train" --rep_test $rep_classes \
                        --param $param_test --classes_1 $classes_1 --classes_2 $classes_2 --net "CNN"
    python $curdir/Scripts/format_data_full_multi_label.py --data_type "test" --rep_test $rep_classes \
                        --param $param_test --classes_1 $classes_1 --classes_2 $classes_2 --net "CNN"
    python $curdir/Scripts/format_data_full_multi_label.py --data_type "valid" --rep_test $rep_classes \
                        --param $param_test --classes_1 $classes_1 --classes_2 $classes_2 --net "CNN"
fi
STEP1_END=$(date +%s)
...
python $pdnndir/cmds/run_CNN.py --train-data "$rep_classes/train.pickle.gz" --multi_label $multi_label \
                                ...
                                --nnet-spec $coucheDNN --wdir $rep_classes --param-output-file  $rep_classes/cnn.param\
                                --l2-reg 0.001 --lrate $lambda --model-save-step 1 --ptr-layer-number 0 \
                                --cfg-output-file $rep_classes/cnn.cfg > $rep_classes/cnn.training.log \
STEP3_END=$(date +%s)
